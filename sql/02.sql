...
select film_id, title from (film right join (select rating, count(rating) from film group by rating order by count desc limit 2)n1 on film.rating=n1.rating) order by title;
